...
export default function ProfileNav({params}) {
  const [activeTab, setActiveTab] = useState('showcase');
  const {userId } = params;
  const { calculateUserMatchStrength, allProjects, allUsers } = useUser();

  // Get user's collection Data
  const currentUser = allUsers.filter(user => user.user_id === userId);
  const userProjects = allProjects.filter(project => project.userId === userId);
  const matchData = currentUser ? calculateUserMatchStrength(currentUser, userProjects) : null;
  if(!currentUser) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {activeTab === 'showcase' && <Showcase userId={userId} />}
        {activeTab === 'skills' && <UserSkill userId={userId} />}
        {activeTab === 'projects' && <UserProject userId={userId} />}
        {activeTab === 'impact' && <Impact userId={userId} />}
      </div>
    </div>
  );
}
